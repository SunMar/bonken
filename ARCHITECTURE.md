...
- **Debug asserts, graceful release.** Invariants use `assert` (compiled out in
  ...
  The same posture covers the bootstrap: `main()` runs the whole startup in a
  `runZonedGuarded` zone and installs `FlutterError.onError` +
  `PlatformDispatcher.instance.onError` as a global error net, so an uncaught
  async error during launch is logged and swallowed rather than crashing the
  process. Each failable bootstrap step is guarded individually too: a settings
  load failure surfaces the storage-error screen (§9), and a `PackageInfo` read
  failure routes to `BootErrorScreen` (§8) instead of fabricating a legacy/normal
  branch.
...
```
lib/
  main.dart                  Entry: guarded bootstrap (runZonedGuarded + FlutterError/
                             PlatformDispatcher error net), the one-off legacy→async prefs
  ...
  state/                     Riverpod providers.
    ...
    storage_exceptions.dart  Shared persistence-error family: PersistenceException sealed base +
                             ...
                             thrown by game-history and settings load (the failing store is known
                             at the catch site).
...
(coalescing keystroke bursts into one SharedPreferences encode), building the
...
it in a single `SharedPreferences` write (`persistSettings`). This is the
...
live under separate SharedPreferences keys with no cross-key transaction, so the
