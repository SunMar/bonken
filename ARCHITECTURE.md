...
source for three pre-sized renders (`generate_icons.sh`), each rendered at its
exact display size so nothing resizes at runtime:
...
- **Share-card icon** — `icon_bonken_share.png` at 72 px (3× its 24 dp display
  ...
  runtime `ClipRRect`.
- **Web favicon** — `web/favicon.png` at 32 px (the browser-tab icon, linked
  from `index.html`); the browser doesn't mask favicons, so the rounding is
  baked in like the others.
...
to regenerate them from offline. They — and the committed README QR code (see the
**Download page** section below) — are the only committed PNGs in the repo.
...
`web/download.html` (served at `/bonken/download.html`) is a static store-redirect
page: it detects the platform and sends iOS visitors to the App Store and Android
visitors to Google Play, falling back to showing both badges on desktop. The README
links a QR code to it, so a single scan routes the scanner to the right store. It is
plain HTML outside the Flutter app — styled to match (a dark Material 3 theme on the
indigo seed) but with its own hand-written CSS — so it can't reach Flutter's bundled
assets.

`generate_icons.sh` therefore stages the page's images into `web/`, where they get
stable sibling URLs: the squircle app icon rendered to `web/download-logo.png`, plus
copies of the `3.0x/` store badges. These are gitignored like the other generated
web assets.

The QR code (`docs/qr-download.png`) is the exception — it is **committed**, because
it has to render in the README on GitHub, where no build step runs. It was generated
once, by hand, and there is deliberately no tool to recreate it: the encoded URL is
stable, so it effectively never changes. If it ever must, render a new PNG from the
new URL (any QR encoder works) and re-commit.
