...
void main() {
  ...
  group('chooserIndexFor', () {
    test('chooser sits to the left of the dealer (dealer + 1, wrapping)', () {
      expect(chooserIndexFor(0), 1);
      expect(chooserIndexFor(1), 2);
      expect(chooserIndexFor(2), 3);
      expect(chooserIndexFor(3), 0);
    });

    test('is the exact inverse of dealerIndexFor', () {
      for (var chooser = 0; chooser < playerCount; chooser++) {
        expect(chooserIndexFor(dealerIndexFor(chooser)), chooser);
      }
    });
  });
